...
can adjust it for the current selection without changing the configured default.
Retrying preparation keeps the adjustment. The source configuration sets the
Theme to 6 seconds and Variation I to 4 seconds.

The Theme's authored 12 phrase slots (eleven four-letter, one five-letter) expand to
98 performed targets over both repeat visits, including six performed holds.
Runtime preparation chooses unique common words, retries boundedly until the
compiled score has no held-key conflicts, and couples the 98 targets to the
staff-1 score notes. A new assignment differs from the preceding assignment.
The source score coordinates, note IDs, repeat policy and accompaniment never
change. Results contain a frozen copy of a human run's words.

Demo uses one ordered attack/release plan. The audio service schedules the full
bounded plan's note-on and hold note-off events on the
synth's audio clock; Canvas frames only apply those same actions to TypingRound
at their authored event times before target expiry. Tap note-off stays at the
mapped score endpoint. Demo reveals a compact, fixed QWERTY typing block during
count-in and playback; it collapses to zero height when Demo ends. Pressed
letters are highlighted for at least 180 real milliseconds, then fade over
110 milliseconds. Holds stay highlighted until their actual release, and a
repeated letter retriggers an outline pulse. This is cosmetic state sampled by
the existing Canvas frame loop; it does not extend sounds or hold credit.
Physical typing and seeking are disabled during Demo. No player Results are
generated by Demo.

The manifest still contains the other scores and their assets for import tests, but
the visible library intentionally exposes only the Theme and Variation I. The lighter
SalC5Light2.sf2 remains the instrument.

Phase 16 checks: the production build, application TypeScript check and 87
focused piano tests passed. Headless Chrome played the complete 48-second
Twinkle demo at 1x and 3x. Each run scheduled 98 melody attacks, showed 98
Perfect judgements, zero Good/Miss/Wrong and 600/600 sustain points, and
returned to armed Play with its words unchanged. During the 3x run, Canvas
frames were delayed for over two seconds; scheduled audio actions and scoring
still completed. Browser checks also covered Results Replay, practice Reroll,
Settings and Escape cancellation, ignored physical input, light/dark keycaps,
and reduced motion. No extra SoundFont request or browser console error was
observed after preparation. These automated checks do not establish audible
quality or perceived audio/visual synchronisation; those need human listening.

Phase 17 checks: production build, application TypeScript check and 92 focused
piano tests passed. Headless Chrome confirmed zero keyboard height in armed and
human Play, a 220 ms count-in expansion, and a clean collapse after a full 3x
demo and after cancellation. The 3x run still reached 98 Perfect, zero
Good/Miss/Wrong and 600/600 sustain points. A rapid cancel/restart preserved
visibility correctly. At 1x, the keyboard remained legible in the dark theme
at a 700 px viewport with no horizontal overflow or Canvas width mismatch.
Light-theme screenshots were inspected for tap, repeated letter and concurrent
hold/tap states; dark-theme screenshots covered tap and concurrent hold/tap.
Reduced motion switched layout immediately. One local SoundFont request and no
browser errors were observed. Audible quality was not assessed.
...
Armed Play also accepts a deliberate key, Space or Enter as
...
and seeked Results, partial sustain, run cleanup, both themes and reduced
...
The `twinkle-theme` manifest entry introduced player-performed melody; Phase 18
also enables it for Variation I.
...
Non-player-performed songs still queue their original imported MIDI and have no player voices.
...
Restart, Exit, seek, song changes, audio failure and teardown release player
voices. Blur and Settings release human-held voices; Phase 18 keeps demo-owned
voices alive through both. Demo uses the same player-performance path as typing,
...
The current Twinkle authoring groups 49 written attacks into eleven four-letter
words and one five-letter word. The immutable musical template is in
charts/twinkle-theme-chart.ts. charts/prepare-twinkle-run.ts creates a seeded,
validated word assignment for each requested run. Both repeat visits reuse the
same words, and ordinary Start, Demo, Settings, resize and seek keep the chart.

## Phase 18: Variation I, continuous demos and scoring

The visible Library now explicitly contains the Theme and **Twinkle — Variation
I**, in that order. Other configured scores and assets remain available to
import tests, but are hidden from the game Library. Each selected score loads
just in time through the existing SoundFont engine. Variation I has a complete
322-attack, 8-hold chart across its 48 performed measures; its original 44 word slots
reroll per run and preserve repeat wording. See [CHART-AUTHORING.md](./CHART-AUTHORING.md)
for phrase choices, ties, alternate endings, density and short-note limits.

Demo now queues the bounded melody and enabled metronome clicks on the audio
clock during count-in. The sequencer keeps handling accompaniment. It does not
depend on animation frames or recurring main-thread timers for demo sound.
Settings and window blur leave demo voices and transport running; returning to
the tab replays due judgement events into the current state without replaying
old sounds or showing expired key flashes. Escape or Restart stops the run.
Queued SpessaSynth note-ons have no cancellation API, so cancelled demo
channels are muted and retired until their last queued event, including across
synth resets; subsequent attempts use available channels. A browser or OS that
suspends the AudioContext freezes its audio clock and can be resumed from the
Play panel. External audio suspension remains platform dependent.

The Play header separates navigation/Settings from transport. The persistent
panel groups Start, Demo, Reroll and the single speed selector; status and
Restart occupy stable slots. The HUD shows score and speed, combo with capped
CSS flames, and quieter progress statistics. The passage stays immediately
above the demo keyboard (when active) and piano roll. Reduced-motion users
retain a static fire level. Results show attacks, sustain, combo bonus,
Miss/Wrong deductions and the attempt's fixed speed multiplier. The maximum
includes a perfect uninterrupted combo, and the displayed score clamps only
after all raw penalties and bonuses are multiplied.

Automated Chrome checks loaded both visible songs and observed all 322
Variation I demo note-ons queued before the first playback frame. They exercised Settings
and blur without cancelling, then backgrounded the tab for several seconds.
The audio-clock position advanced and the visual judgement caught up on return
without console errors. Both 3× demos reached their natural end; a cancelled
Theme demo returned to Ready with future channels retired. Screenshots were
inspected at desktop and smaller widths in armed, count-in and demo states,
light/dark themes and reduced motion. This checks event scheduling and browser
state, not human listening or musical feel.

## After Phase 18: letter layout, look-ahead and Variation I words

Streaming letters and their hit effects now share one horizontal Canvas line
by default (y = 82). **Settings → Gameplay & view → Stagger streaming letters**
restores the earlier alternating rows (y = 67 and 96). The switch takes effect
while the page is open and clears an in-progress hit effect at the old position;
it changes only drawing, not note timing, audio or judgement. The setting
defaults to off on a new page load.

`song-library.config.cjs` now gives every configured song a `defaultLookAhead`.
The generator validates the slider's 2–10-second range and 0.5-second steps,
then writes the value to `song-manifest.generated.ts`. Selecting a song applies
its default; changing the Settings slider overrides it for that selection, and
retrying preparation keeps the override. Selecting a song again reapplies its
configured default. Theme is configured for 6 seconds, Variation I for 4, and
the three hidden scores for 6. Edit the config and run
`node src/app/piano/generate-song-manifest.cjs` to change a default; the
generated file is not the source of truth. The npm start/build/test scripts
also regenerate it.

Variation I's 19 fast written bars now each use one word rather than two
half-bar words. Fifteen full eighth-note bars use eight-letter words; measures
29, 30, 46 and 47 use seven-letter words because a tied continuation is not a
new attack. The authored template therefore has 25 word slots instead of 44.
The shared randomizer has 22 familiar seven-letter and 27 eight-letter choices,
without repeating a long word within one assignment. The 322 performed
attacks, eight holds, note endpoints, accompaniment and repeat expansion are
unchanged. See [CHART-AUTHORING.md](./CHART-AUTHORING.md) for the musical mapping.

Verification since Phase 18: application and spec TypeScript checks passed;
the production build passed after the layout toggle. A Chrome check saw the
letter rows switch from `[82, 82]` to `[67, 96]`. The three focused Variation I
chart tests passed, including complete melody coverage and reroll invariants.
The most recent complete piano suite had 98 passes and two metadata failures:
the generated manifest currently lists Theme look-ahead as 5 seconds and
Variation I difficulty as Advanced, while the source config and existing tests
say 6 seconds and Intermediate. Regenerating the manifest will replace those
