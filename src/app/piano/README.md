...
## Current Play flow (Phase 17)

Run npm start and open http://localhost:4200/piano. Start opens the library;
only Twinkle Twinkle Little Star is currently exposed. Selecting it prepares
...
Escape during Demo, Settings during Demo, and window blur during Demo return to
armed Play with the same words. A practice seek keeps the current words; Demo
starts at zero even after a seek. Exit and Home silence the run.

The authored 12 phrase slots (eleven four-letter, one five-letter) expand to
...
Demo uses one ordered attack/release plan. The audio service looks ahead from
AudioContext/sequencer time and schedules note-on and hold note-off on the
...
The manifest still contains four scores and their assets for import tests, but
the visible library intentionally exposes only Twinkle. The lighter
...
release player voices. Demo uses the same player-performance path as typing, scheduled on the audio clock.
